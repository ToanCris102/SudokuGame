...
function updateMove(tile) {
    ...
    if(selectedTile && selectedNum && (Number(selectedNum.textContent) > 0 && Number(selectedNum.textContent) <= 9)) {        
        ...
        if(pen === true) {                
            ...
            selectedTile.style.fontSize = "default"; 
...
function createSolution() {
    if(confirm("There is only one thing that makes a dream impossible to achieve: the fear of failure.")){
        for(let i = 0; i < 81; i++) {
            if(qsa(".tile")[i].textContent == '' || qsa(".tile")[i].style.fontSize == "8pt"){
                qsa(".tile")[i].style.fontSize = 'default';
